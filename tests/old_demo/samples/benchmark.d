...
static int simple_terrain_count = simple_terrain_verts.length;
...
//	cpBody *body = cpSpaceAddBody(space, cpBodyInit(&bodies[i], mass, cpMomentForCircle(mass, 0.0f, radius, cpvzero)));
    _body.p = cpvmult(frand_unit_circle(), 180.0f);
...
static void add_box(int i, cpFloat size){
    ...
    cpBody *_body = cpSpaceAddBody(space, cpBodyNew(mass, cpMomentForBox(mass, size, size)));
//	cpBody *body = cpSpaceAddBody(space, cpBodyInit(&bodies[i], mass, cpMomentForBox(mass, size, size)));
    _body.p = cpvmult(frand_unit_circle(), 180.0f);


    cpShape *shape = cpSpaceAddShape(space, cpBoxShapeNew(_body, size, size));
    shape.e = 0.0f; shape.u = 0.9f;
}

static void add_hexagon(int _i, cpFloat radius){
    cpVect hexagon[6];
    for(int i=0; i<6; i++){
        float angle = -cast(float)PI*2.0f*i/6.0f;
        hexagon[i] = cpvmult(cpv(cos(angle), sin(angle)), radius);
    }

    cpFloat mass = radius*radius;
    cpBody *_body = cpSpaceAddBody(space, cpBodyNew(mass, cpMomentForPoly(mass, 6, hexagon.ptr, cpvzero)));
//	cpBody *body = cpSpaceAddBody(space, cpBodyInit(&bodies[i], mass, cpMomentForPoly(mass, 6, hexagon, cpvzero)));
    _body.p = cpvmult(frand_unit_circle(), 180.0f);

    cpShape *shape = cpSpaceAddShape(space, cpPolyShapeNew(_body, 6, hexagon.ptr, cpvzero));
    shape.e = 0.0f; shape.u = 0.9f;
}
...
static cpSpace *init_SimpleTerrainHexagons_1000(){
    setupSpace_simpleTerrain();
    for(int i=0; i<1000; i++) add_hexagon(i, 5.0f);
...
static cpSpace *init_SimpleTerrainHexagons_500(){
    setupSpace_simpleTerrain();
    for(int i=0; i<500; i++) add_hexagon(i, 5.0f);
...
static cpSpace *init_SimpleTerrainHexagons_100(){
    setupSpace_simpleTerrain();
    for(int i=0; i<100; i++) add_hexagon(i, 5.0f);
...
static int complex_terrain_count = complex_terrain_verts.length;
...
static cpSpace *init_ComplexTerrainCircles_1000(){
    ...
    for(int i=0; i<1000; i++){
        ...
        cpFloat mass = radius*radius;
        cpBody *_body = cpSpaceAddBody(space, cpBodyNew(mass, cpMomentForCircle(mass, 0.0f, radius, cpvzero)));
        _body.p = cpvadd(cpvmult(frand_unit_circle(), 180.0f), cpv(0.0f, 300.0f));
...
static cpSpace *init_ComplexTerrainHexagons_1000(){
    ...
    cpFloat radius = 5.0f;
    cpVect hexagon[6] = void;
    for(int i=0; i<6; i++){
        cpFloat angle = -PI*2.0f*i/6.0f;
        hexagon[i] = cpvmult(cpv(cos(angle), sin(angle)), radius);
    }
...
static int bouncy_terrain_count = bouncy_terrain_verts.length;
...
static cpSpace *init_BouncyTerrainCircles_500(){
    ...
    for(int i=0; i<400; i++){
        ...
        cpFloat mass = radius*radius;
        cpBody *_body = cpSpaceAddBody(space, cpBodyNew(mass, cpMomentForCircle(mass, 0.0f, radius, cpvzero)));
        _body.p = cpvadd(cpvmult(frand_unit_circle(), 130.0f), cpvzero);
        _body.v = cpvmult(frand_unit_circle(), 50.0f);
...
static cpSpace *init_NoCollide(){
    ...
    for(int y=10-240; y<=240; y+=40){
        cpFloat mass = 7.0f;
        cpBody *_body = cpSpaceAddBody(space, cpBodyNew(mass, cpMomentForCircle(mass, 0.0f, radius, cpvzero)));
        _body.p = cpv(-320.0f, y);
        _body.v = cpv(100.0f, 0.0f);
    ...
    for(int x=30-320; x<=320; x+=40){
        cpFloat mass = 7.0f;
        cpBody *_body = cpSpaceAddBody(space, cpBodyNew(mass, cpMomentForCircle(mass, 0.0f, radius, cpvzero)));
        _body.p = cpv(x, -240.0f);
        _body.v = cpv(0.0f, 100.0f);
...
string bench(in string _name)
{
    return "{ \"benchmark - " ~_name~ "\", null, &init_" ~ _name ~ ", &update, &destroy},";
}

//pragma(msg, bench("SimpleTerrainCircles_1000"));

string benchmarks(string[] _names)
{
    string res = "enum chipmunkDemo[] bench_list = [";
...
//pragma(msg, benchmarks(benchmarknames));

mixin(benchmarks(benchmarknames));
